...
type getClassroomStudentsQuery = {
  startPeriod: Date;
  endPeriod: Date;
  activeOnly?: boolean;
};

const getClassroomStudentsQuerySchema = Joi.object<getClassroomStudentsQuery>({
  startPeriod: Joi.date().required(),
  endPeriod: Joi.date().required(),
  activeOnly: Joi.boolean().optional(),
});

const getClassroomStudentsParamsSchema = Joi.object({
  classroomId: Joi.string().required(),
});

export default async function getClassroomStudentsInfo(
  req: Request,
  res: Response,
  next: NextFunction
) {
  const error = joiValidator(getClassroomStudentsParamsSchema, "params", req, res);
  if (error) {
    next(error);
    return;
  }

  const queryError = joiValidator(getClassroomStudentsQuerySchema, "query", req, res);
  if (queryError) {
    next(queryError);
    return;
  }
