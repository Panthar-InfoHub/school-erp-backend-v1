...
export default async function getClassroomSectionStudentsInfo(
	...
	try {
		...
		const enrollments = await StudentEnrollment.findAll({
			...
			attributes: ["id", "studentId", "sessionStart", "sessionEnd", "monthlyFee", "isActive"],
			include: [
				{
					model: Student,
					attributes: {
				...
				},
				{
					model: ClassSection,
					attributes: {
						exclude: ["studentId", "classroomId", "monthlyFee", "createdAt", "updatedAt", "subjects"],
		...
		const processedData = enrollments.map((enrollment) => {
			...
			return {
				...
				feeDueTotal,
				feeCompletelyPaid,
				lastPaymentDate, // will be null if there's no payment date found
			};
		});
