...
export default async function updateVehicle(
  ...
  try {
    ...
    // If driverId is provided for update
    if (driverId !== undefined) {
      // Find the current driver linked to this vehicle.
      const currentDriver = await Driver.findOne({
        where: { vehicle_id: vehicleId },
        transaction,
      });

      // Check if the provided driverId is the same as the currently linked one.
      if (currentDriver && currentDriver.id === driverId) {
        logger.info(
          `Driver ${driverId} is the same as the current driver; no change in driver linking.`
        );
      } else {
        // If a driver is currently linked, detach by setting their vehicle_id to null.
        if (currentDriver) {
          logger.info(
            `Detaching current driver ${currentDriver.id} from vehicle ${vehicleId}.`
          );
          await currentDriver.update({ vehicle_id: null }, { transaction });
        }

        // Check if the new driver id is non-empty. An empty string signals detachment.
        if (driverId) {
          ...
          if (!newDriver) {
            await transaction.rollback();
            logger.error(`New driver not found: ${driverId}`);
            return next(
              new ResponseErr(
                404,
                "Driver Not Found",
                "The new driver id provided does not exist."
              )
            );
          }

          // Verify if the new driver already has a vehicle assigned
          if (newDriver.vehicle_id) {
            await transaction.rollback();
            logger.error(`New driver already has a vehicle: ${driverId}`);
            return next(
              new ResponseErr(
                409,
                "Conflict",
                "The new driver already has a vehicle assigned to them."
              )
            );
          }

          logger.info(`Linking new driver ${driverId} to vehicle ${vehicleId}.`);
          await newDriver.update({ vehicle_id: vehicleId }, { transaction });
          vehicle.driverId = driverId;
        } else {
          // If an empty driverId is provided, clear the vehicle's driver linking.
          logger.info(`Clearing driver linking from vehicle ${vehicleId}.`);
          vehicle.driverId = "";
        }
      }
    }
