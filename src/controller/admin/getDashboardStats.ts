...
export async function getDashboardStatus(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    // 1. Count total teachers and employees
    ...
    const totalRegisteredStudentsInDB = await Student.count()
    const totalActiveRegisteredStudent = await Student.count({
      where: {
        isActive: true
      }
    })
    
    const enrollmentsCreatedInLastThirtyDays = await StudentEnrollment.count({
      where: {
        createdAt: {
    ...
    // Add this type definition to handle the raw query result
    type AttendanceDayResult = {
      day: string;
      attendanceCount: string;
    };
    
    const averageEmployeeAttendanceRateInLastThirtyDays = await EmployeeAttendance.findAll({
      where: {
        date: {
          [Op.between]: [thirtyDaysAgo, currentDate]
        }
      ...
      attributes: [
        ...
        [sequelize.fn('COUNT', sequelize.col('employeeId')), 'attendanceCount']
      ],
      group: [sequelize.fn('DATE', sequelize.col('date'))],
      raw: true
    }) as unknown as AttendanceDayResult[];

// Calculate average attendance rate
    const totalDays = averageEmployeeAttendanceRateInLastThirtyDays.length;
    let totalAttendanceRate = 0;
    let employeeAverageAttendanceRate = 0;
    
    if (totalDays > 0) {
      for (const dayRecord of averageEmployeeAttendanceRateInLastThirtyDays) {
        const attendanceRate = (parseInt(dayRecord.attendanceCount) / totalEmployees) * 100;
        totalAttendanceRate += attendanceRate;
      }
      
      // Calculate average attendance rate as a percentage
      employeeAverageAttendanceRate = Math.round(totalAttendanceRate / totalDays);
    } else {
      // No attendance records in the last 30 days
      employeeAverageAttendanceRate = 0;
    }
...
// Filter out entries whose associated student enrollment is inactive.
    ...
    res.status(200).json({
      ...
      presentTeacherCount: teacherPresence.filter((teacher) => teacher.present).length,
      totalDuePayment,
      totalPaidLast30Days,
      totalVehicles,
      totalEmployees,
      totalRegisteredStudentsInDB,
      totalActiveRegisteredStudent,
      enrollmentsCreatedInLastThirtyDays,
      employeeAverageAttendanceRate,
